...
func serialiser( ctx *context, master_sheep *bleater.Bleater ) {
	...
	for {
		...
		resp.Rdata = fmt.Sprintf( `{ "sender": %q, "state": "ERROR", "msg_key": %q, "msg": "request timeout" }`, ctx.sid, resp.Msg_key )		// default message when waiting; possibly overwritten below
		...
		if action != nil {
			...
			switch *action {
				...
				case "ping":						// send ping to VFd; see "Ping" for a ping we respond to
					sheep.Baa( 1, "sending ping" )
					fifo_buffer = mk_vfd_request( "ping", nil, nil, ctx.resp_fifo, vfd_rid )

					nw, err := fifo.Write( []byte( fifo_buffer ) )
					if err != nil {
						sheep.Baa( 0, "attempt to write to fifo failed: n=%d %s", nw, err )
						resp.Rdata = fmt.Sprintf( `{ "sender": %q, "state": "ERROR", "msg_key": %q, "msg": "request dropped", "reason": "unable to send request to VFd: %s" }`, ctx.sid, *msg_key, err )
					} else {
				...
				case "add":
					...
					if target != nil {
						...
						if ok {
							...
							if err == nil {
								...
								nw, err := fifo.Write( []byte( fifo_buffer ) )
								if err != nil {
									sheep.Baa( 0, "add: attempt to write to fifo failed: n=%d %s", nw, err )
									reason = "unable to send request to VFd: " + fmt.Sprintf( "%s", err )
								} else {
									sheep.Baa( 2, "request sent: %s", fifo_buffer )
									resp.Wait = true				// request sent, responder should wait for answer
								}
							} else {
					...
					if !resp.Wait {					// error, build a proper response
						resp.Rdata = fmt.Sprintf( `{ "sender": %q, "state": "ERROR", "msg_key": %q, "msg": "add request dropped", "reason": %q }`, ctx.sid, *msg_key, reason )
					}
				...
				case "del", "delete":
					...
					if target != nil {
						...
						if fname != "" {
							...
							nw, err := fifo.Write( []byte( fifo_buffer ) )
							if err != nil {
								sheep.Baa( 0, "delete: attempt to write to fifo failed: n=%d %s", nw, err )
								reason = "unable to send request to VFd: " + fmt.Sprintf( "%s", err )
							} else {
								sheep.Baa( 2, "request sent: %s", fifo_buffer )
								resp.Wait = true				// request sent, responder should wait for answer
					...
					if !resp.Wait {					// error, build a proper response to be sent immediately
						resp.Rdata = fmt.Sprintf( `{ "sender": %q, "state": "ERROR", "msg_key": %q, "msg": "delete request dropped", "reason": %q }`, ctx.sid, *msg_key, reason )
					}

				case "show":
					reason := ""
					fifo_buffer = mk_vfd_request( "show", nil, target, ctx.resp_fifo, vfd_rid )
					nw, err := fifo.Write( []byte( fifo_buffer ) )
					if err != nil {
						sheep.Baa( 0, "show: attempt to write to fifo failed: n=%d %s", nw, err )
						reason = "unable to send request to VFd: " + fmt.Sprintf( "%s", err )
					} else {
						sheep.Baa( 2, "request sent: %s", fifo_buffer )
						resp.Wait = true				// request sent, responder should wait for answer
					...
					if !resp.Wait {					// error, build a proper response to be sent immediately
						resp.Rdata = fmt.Sprintf( `{ "sender": %q, "state": "ERROR", "msg_key": %q, "msg": "show request dropped", "reason": %q }`, ctx.sid, *msg_key, reason )
					}

				default:
					resp.Rdata = fmt.Sprintf( `{ "sender": %q, "state": "ERROR", "msg_key": %q, "msg": "request dropped", "reason": "unknown action: %s" }`, ctx.sid, *msg_key, *action )
			}
		} else {
							...
							switch *action {							// VFd may communicate things other than responses
								case "response":
									...
									if resp != nil {
										...
										msg := jtree.Get_string( "msg" )				// if VFd put a message string in, we'll pull it up too, but likley an array of strings which we don't promote
										rbuf := build_response( ctx.sid, state, msg, &resp.Msg_key, jtree )		// create a response using the user supplied key, and stuffing in the vfd response as data
